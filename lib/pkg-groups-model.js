...
export default class PkgGroupsModel {
  ...
  constructor (serialized) {
     ...
     */
    this.disabled = []
    for (const group of initData['groups']) {
      if (group instanceof PkgGroupsGroup) {
        this.groups[group.name] = group
      } else {
        console.log(group)
        const newGroup = new PkgGroupsGroup(group)
  ...
  /** required for display in a pane: https://atom.io/docs/api/v1.21.1/ViewRegistry */
  getTitle () {
    return 'pkg-groups'
  }
  ...
  groupState (groupName) {
    ...
    for (let [key, value] of this.metas) { // eslint-disable-line no-unused-vars
  ...
  addGroup (groupName, memberIter) {
    ...
    let newGroup = new PkgGroupsGroup({name: groupName, type: 'group', packages: memberIter})
    this.groups = this.groups.set(groupName, newGroup)
