...
export default class PkgPickList {
  ...
  static propFilter = new Immutable.Set(['rightList', 'leftList', 'rightSelected', 'leftSelected', 'rightLabel', 'leftLabel', 'className'])

  didClickRightItem = (item) => this.move('left', item)

  didClickLeftItem = (item) => this.move('right', item)
   ...
   * @emits change
   *
   * @return {Promise} the promise from `update()`, or a rejected Promise if
   *   error occurred.
   */
  move (direction, item) {
    if (direction === 'right') {
      if (!(this.props.leftList.includes(item))) {
        return Promise.reject(new Error(`value ${item} not found in left list`))
      }
      ...
      if (!(this.props.rightList.includes(item))) {
        return Promise.reject(new Error(`value ${item} not found in right list`))
      }
