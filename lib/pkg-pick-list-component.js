 ...
 * @param {?integer} props.rightSelected - index of current selection in rightList, if any
 * @param {?integer} props.leftSelected - index of current selection in leftList, if any
 * @param {?string} props.leftLabel - label applied to left list
 * @param {?string} props.rightLabel - label applied to right list
 * consumes MouseEvent on buttons
...
export default class PkgPickList {
  ...
  /**
   * Establish `newHandler` as a handler function for the 'change' event.
   * @param {function([direction: string, item: string])} newHandler An event
   *   handler function. `direction` is the side to which an item is moved
   *   ('right' or 'left'), `item` is the item selected for move
   * @return {Disposable} call `dispose()` to stop handling event.
   */
  onChange = newHandler => {
    logger.debug(`setting ${newHandler} as a handler for 'change'`)
    this._emitter.on('change', newHandler)
  }
  ...
  move (direction, item) {
    ...
    logger.debug(`emitting 'change' event [${direction}, ${item}]`)
    this._emitter.emit('change', [direction, item])
  ...
  render () {
    ...
    return <div {...passThroughProps}>
      <div className='pick-list-left-list'>
        <p className='pick-list-header'>{this.props.leftLabel ? this.props.leftLabel : ''}</p>
        <PkgSelectList didChangeSelection={this.didClickLeftItem} items={leftItems} emptyMessage={'no packages found'} />
      </div>
      <div className='pick-list-right-list'>
        <p className='pick-list-header'>{this.props.rightLabel ? this.props.rightLabel : ''}</p>
        <PkgSelectList didChangeSelection={this.didClickRightItem} items={rightItems} emptyMessage={'no packages selected'} />
      </div>
    </div>
  }
