...
export default class PkgGroupsView {
  constructor (props, children) {
    ...
    this.pkgCount = 0
    this.props['pkgCount'] = this.pkgCount
        ...
        <div class='inset-panel padded'>
          ...
          <div class='inset-panel padded'>
            ...
            <section className='sub-section installed-packages'>
              <div className='alert alert-info loading-area icon icon-hourglass'>Loading packages…</div>
            </section>
          </div>
        </div>
        <p id='pkg_count'>There are {this.pkgCount} avallable packages.</p>
      </atom-panel>
    </div>
  ...
  /**
   * Loads list of packages using package manager
   */
  loadPackages () {
