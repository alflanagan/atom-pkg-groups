...
export default class PkgListComponent {
  ...
  render () {
    ...
    if (litems.length === 0) {
      ...
      let placeholderClasses = 'alert alert-info loading-area icon icon-hourglass'
      if (this.props['className']) {
        placeholderClasses = `${placeholderClasses} ${this.props['className']}`
      }
      this.element = <div className={placeholderClasses}>Loading packages…</div>
    } else {
      logger.debug(this.props)
