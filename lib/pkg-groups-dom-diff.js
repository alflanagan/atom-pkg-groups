...
import {arrayEqual, objectEqual} from './pkg-groups-functions'
...
export default class DomDiff {
  constructor (dom1, dom2) {
    ...
    this.domTag = dom1.tag
    if (typeof this.domTag === 'function') {
      this.domTag = `function ${this.domTag.name}`
    }
    ...
    for (const key in dom1.props) {
      if (dom2.props && dom2.props.hasOwnProperty(key)) {
        const val1 = dom1.props[key]
        const val2 = dom2.props[key]
        if (val1 !== val2 && !arrayEqual(val1, val2) && !objectEqual(val1, val2)) {
          this.props[key] = [val1, val2]
        }
      } else {
  ...
  toString () {
    ...
    if (this.in1Only && this.in1Only.length > 0) {
      diffList.push(`${tabs}Properties found in 1st DOM (${this.domTag}) but not the second: ${this.in1Only}`)
    }
    ...
    if (this.childCount[0] !== this.childCount[1]) {
      ...
      try {
        tabLevel += 1
        for (const key in this.children) {
          diffList.push(`${tabs}The DOMs (${this.domTag}) have differing children at index ${key}. Differences:`)
